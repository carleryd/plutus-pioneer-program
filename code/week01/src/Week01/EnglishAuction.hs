...
import Playground.Contract (IO, ensureKnownCurrencies, printJson, printSchemas, stage)
...
-- `Datum` holds the state of the contract UTXO. In this case it's info about the highest bidder
-- `Redeemer` is the thing that is triggered on the contract UTXO, it's the one thing that can be
--   interacted with.
-- The "on-chain" contract is just a function returning a Bool. It can be thought of as a checker
--   run everytime a user tries to call a Redeemer on the contract.

-- Scenario: User calls `Redeemer` which may:
--   * change the state i.e. `Datum` of the contract
--   * consume other UTXOs and create new UTXOs from the contract
-- This is checked with the "validator" or "on-chain contract" function. If that function returns
--   "True" then the contract UTXO is consumed and a new contract UTXO is created for further consumption
-- This new contract UTXO may have different `Datum` and may have caused changes to other UTXOs.
...
mkAuctionValidator ad redeemer ctx =
  traceIfFalse "wrong input value" correctInputValue
    && case redeemer of
      MkBid b@Bid {..} ->
        traceIfFalse "bid too low" (sufficientBid bBid) -- Makes sure bid is highest
          && traceIfFalse "wrong output datum" (correctBidOutputDatum b)
          && traceIfFalse "wrong output value" (correctBidOutputValue bBid)
          && traceIfFalse "wrong refund" correctBidRefund -- Refunds the previous highest bidder
          ...
          && case adHighestBid ad of
            Nothing ->
              traceIfFalse
                ...
                (getsValue (aSeller auction) $ tokenValue <> Ada.lovelaceValueOf minLovelace)
            Just Bid {..} ->
              traceIfFalse
  ...
  let b = Bid {bBidder = pkh, bBid = bpBid}
      ...
      lookups =
        Constraints.typedValidatorLookups typedAuctionValidator
          P.<> Constraints.otherScript auctionValidator
